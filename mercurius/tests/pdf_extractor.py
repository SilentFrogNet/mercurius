...
from mercurius.extractors import PDFExtractor
...
class TestPDFExtractor(RootTest):

    def test_01_parse_wrong_file(self):
        result = self.im.parse_data(**self.wrong_file)
        self.assertIsNone(result)

    def test_02_parse_correct_file(self):
        result = self.im.parse_data(**self.correct_file)
        self.assertIsInstance(result, PDFExtractor)

    def test_03_parse_with_user_file(self):
        result = self.im.parse_data(**self.with_user_file)
        self.assertIsInstance(result, PDFExtractor)
        self.assertListEqual(result.get_users(), self.with_user_file_data['users'])

    def test_04_parse_with_all_file(self):
